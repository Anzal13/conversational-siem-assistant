...
import { getMetrics, generateTrafficData, getAttackDistribution, generateIncidents } from '@/lib/mock-data';
...
export default function Overview() {
  ...
  useEffect(() => {
    const iv = setInterval(() => setMetrics(getMetrics()), 5000);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Real-time threat intelligence powered by Random Forest & Isolation Forest ML</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <MetricCard title="Total Traffic" value={metrics.totalTraffic.toLocaleString()} icon={<Activity className="h-5 w-5" />} variant="primary" trend={{ value: 12, label: 'vs last hour' }} />
        <MetricCard title="Detected Threats" value={metrics.detectedThreats} icon={<Shield className="h-5 w-5" />} variant="critical" trend={{ value: 8, label: 'vs last hour' }} />
        <MetricCard title="Anomaly Score" value={metrics.avgAnomalyScore} subtitle="Isolation Forest avg" icon={<Brain className="h-5 w-5" />} variant="warning" />
        <MetricCard title="System Uptime" value={`${metrics.systemUptime}%`} icon={<Server className="h-5 w-5" />} variant="success" />
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <MetricCard title="Active Alerts" value={metrics.activeAlerts} icon={<AlertTriangle className="h-5 w-5" />} variant="warning" />
        <MetricCard title="Blocked Attacks" value={metrics.blockedAttacks.toLocaleString()} icon={<Lock className="h-5 w-5" />} variant="success" />
        <MetricCard title="ML Accuracy" value={`${metrics.mlAccuracy}%`} subtitle="RF + IF combined" icon={<Zap className="h-5 w-5" />} variant="primary" />
        <MetricCard title="Avg Response" value={`${metrics.responseTime}ms`} icon={<Clock className="h-5 w-5" />} />
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        ...
        <div className="bg-card rounded-lg border border-border p-4">
          ...
          <ResponsiveContainer width="100%" height={280}>
            <PieChart>
              <Pie data={attacks} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={90} innerRadius={50} paddingAngle={2} strokeWidth={0}>
                {attacks.map((entry, i) => (
